...
class SS2D_conv(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        B, C, H, W = x.shape
        q1, q2, q3, q4 = self.forward_core(x)
...
class EVSSBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        y = y * F.silu(z)
        out = self.drop_path(self.out_proj(y))
        out = out + self.skip_scale * input
...
if __name__ == '__main__':
    x = torch.randn(1, 16, 64, 64).cuda()  # B C V W H
    slicing = EVSSBlock(16).cuda()
    y = slicing(x)
